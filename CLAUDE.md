...
- **One report, many readers.** `getOrBuildReport()` in [lib/report.ts](lib/report.ts) is the single path to opponent data: it returns the saved report, rebuilding from Leaguepedia only when it is missing, older than three days, or refreshed. `/api/opponent-report` (Prep), `/api/scout` (the draft room's panel, mapped to the shape it renders) and the daily cron all go through it, so an opponent is fetched once. Team-name aliases (`BIG` → `Berlin International Gaming`) live in `buildReport`.
