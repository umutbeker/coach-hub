...
- **One report, many readers.** `getOrBuildReport()` in [lib/report.ts](lib/report.ts) is the single path to opponent data: it returns the saved report, rebuilding from Leaguepedia only when it is missing, older than three days, refreshed, or built to an **older `ANALYSIS_VERSION`** — the same reason `/pro` bumps its cache key. Checking only that `draft` exists was not enough: a report built before `comps` was added passed that test and reached a page reading `d.comps.won`, which is a client-side crash, not a missing section. Bump `ANALYSIS_VERSION` in [lib/report.ts](lib/report.ts) whenever `DraftAnalysis` changes shape, and keep the page rendering each section only when its data is there, so the next mismatch degrades instead of white-screening. `/api/opponent-report` (Prep), `/api/scout` (the draft room's panel, mapped to the shape it renders) and the daily cron all go through it, so an opponent is fetched once. Team-name aliases (`BIG` → `Berlin International Gaming`) live in `buildReport`.
